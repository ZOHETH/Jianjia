...
namespace jjf {
	namespace utils {
		/**
		 * Vulkan offers various "memory types" with unique characteristics suitable for different tasks on the GPU.
		 * Selecting the right memory type is essential for optimal performance and functionality.
		 *
		 * This function, `GetVulkanMemoryType`, helps find an appropriate memory type based on desired properties.
		 * For example, when allocating a texture, one might seek a device-local memory type for fastest access.
		 */
		...
		static VkFormat JjFormatToVulkanFormat(ImageFormat format)
		{
			switch (format)
			{
		...
		: m_Filepath(path)
	{
		...
		{
			data = (uint8_t*)stbi_loadf(m_Filepath.c_str(), &width, &height, &channels, STBI_rgb_alpha);
	...
	void Image::SetData(const void* data)
	{
		...
		// Copy to Image
		{
			...
			copy_region.imageExtent.width = m_Width;
			copy_region.imageExtent.height = m_Height;
			copy_region.imageExtent.depth = 1;
			vkCmdCopyBufferToImage(command_buffer, m_StagingBuffer, m_Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy_region);
	...
	void Image::AllocateMemory(uint64_t size)
	{
		...
		VkFormat vulkanFormat = utils::JjFormatToVulkanFormat(m_Format);
